...
function _M:load_backup()
  if config_loaded then
    ngx_log(ngx_NOTICE, "load backup called but config already exists")
    return
  end
  local file, err = io.open("/opt/bitnami/backup/config.json", "r")
  if file then
    local backup_config, err = file:read("a")

    if err then
      ngx_log(ngx_ERR, err)
    end
    local config = assert(cjson_decode(backup_config))

    local ok, err, err_t = config_helper.update(self.declarative_config, config)

    if ok then
      ngx_log(ngx_NOTICE, "loaded backed up configuration")
    else
      ngx_log(ngx_ERR, _log_prefix, "unable to update running config: ", err_t)
      ngx_log(ngx_ERR, _log_prefix, "unable to update running config: ", err)
    end
  else
    ngx_log(ngx_ERR, err)
    ngx_log(ngx_ERR, "failed to open file for write!")
  end
  config_loaded = true
end
...
function _M:communicate(premature)
  ...
  local _
  _, err = c:send_binary(cjson_encode({
    type = "basic_info",
  ...
  local config_thread = ngx.thread.spawn(function()
    while not exiting() and not config_exit do
      ...
      local filewrite, err = io.open("/opt/bitnami/config.json", "w")
      if filewrite then
        filewrite:write(cjson_encode(msg))
        filewrite:close()
      else
        ngx_log(ngx_ERR, err)
        ngx_log(ngx_ERR, "failed to open file for write!")
